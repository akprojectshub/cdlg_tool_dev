...
param_file = 'data/parameters/parameters_gradual_drift'
...
def generate_log_with_gradual_drift(file_path_one=None, file_path_two=None):
    ...
    :return: event log with gradual drift saved in out_file
    """
    print('Generating event log with a gradual drift based on', param_file)
    out_file = os.path.join(out_folder, 'param_log_gradual_' + str(int(time.time())) + '.xes')
    ...
    if file_path_two is None:
        drift_data = "perspective: control-flow; type: gradual; specific_information: "+change_type.strip()+" distribution; drift_start: "+str(start_drift)+" (" + str(start_point) + "); drift_end: "+str(end_drift)+" (" + str(end_point) + "); activities_added: "+str(added_acs)+"; activities_deleted: "+str(deleted_acs)+"; activities_moved: "+str(moved_acs)
    else:
        drift_data = "perspective: control-flow; type: gradual; specific_information: "+change_type.strip()+" distribution; drift_start: "+str(start_drift)+" (" + str(start_point) + "); drift_end: "+str(end_drift)+" (" + str(end_point) + ")"
...
def get_parameters():
    """ Getting parameters from the text file 'parameters_gradual_drift' placed in the folder 'Data/parameters'

    :return: parameters for the generation of an event log with a gradual drift
    """
    ...
    change_type = doc.readline().split(' ')[1]
    ...
    return tree_complexity, date, min_sec, max_sec, nu_traces, start_point, end_point, change_type, proportion_random_evolution, start_sector_noise, end_sector_noise, proportion_noise_in_sector, type_noise
...
def main():
    ...
    if len(sys.argv) == 1:
        generate_log_with_gradual_drift()
    elif len(sys.argv) == 2:
        generate_log_with_gradual_drift(sys.argv[1])
    else:
        generate_log_with_gradual_drift(sys.argv[1], sys.argv[2])
