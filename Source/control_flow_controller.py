...
from Source.input_controller import input_int, input_ac, input_type_cf, input_ops, input_yes_no, input_cft_ac, \
    input_cft_op, input_cft_tf, input_ra_ch, input_per_not_null, input_percentage
from Source.process_tree_controller import swap_two_existing_activities, visualise_tree, \
...
def change_tree_on_control_flow(tree):
    ...
    if ran == 'random':
        ...
        return evolve_tree_randomly(drift_tree, evolution_stage)
    else:
        ...
        while continue_evolve == 'yes':
            ...
            control_flow_type = input_type_cf(
                "Operator for the " + str(
                    i) + ". intermediate evolution of the process tree version \n[change_activity, change_operator, change_tree_fragment, delete_silent_ac]: ")
            drift_tree = control_flow_change(drift_tree, control_flow_type)
    ...
    while continue_evolve == 'yes':
        ...
        control_flow_type = input_type_cf(
            "Operator for the " + str(i) + ". intermediate evolution of the " + str(
                round_ev) + ". evolving model version [change_activity, change_operator, change_tree_fragment, delete_silent_acx]: ")
        drift_tree = control_flow_change(drift_tree, control_flow_type)
        visualise_tree(drift_tree)
    ...
    if control_flow_type == 'change_activity':
        ...
        if cft_ac_type == 'swap_acs':
            ...
            act_two = input_ac("Second activity to be swapped " + str(drift_tree._get_leaves()) + ": ",
                               drift_tree._get_leaves())
            swap_two_existing_activities(drift_tree, act_one.strip(), act_two.strip())
        elif cft_ac_type == 'delete_ac':
            activity = input_ac("Activity to be deleted " + str(drift_tree._get_leaves()) + ": ",
                                drift_tree._get_leaves())
            delete_activity(drift_tree, activity.strip())
            ...
            replace_activity(drift_tree, act_one, act_two)
            ...
            drift_tree = add_activity(drift_tree, act, depth)
        ...
        if cft_op_type == 'replace_op':
            ...
            replace_operator(drift_tree, depth_node, old_operator)
            ...
            swap_operator(drift_tree, depth_first, operator_second, operator_first, ac_one)
            swap_operator(drift_tree, depth_second, operator_first, operator_second, ac_two)
        ...
        if cft_tf_type == 'add_fragment':
            ...
            drift_tree = add_activity_with_operator(drift_tree, activity_added, operator_added, depth, direction)
            ...
            drift_tree = delete_part_tree(drift_tree, depth, operator_deleted)
        elif cft_tf_type == "swap_fragments":
            drift_tree = swap_tree_fragments(drift_tree)
            ...
            drift_tree = move_tree_fragment(drift_tree, operator_start, operator_depth)
    else:
        operator = input_ops(
